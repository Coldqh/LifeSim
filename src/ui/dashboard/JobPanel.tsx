...
export function JobPanel({ currentJobView, onWorkShift }: JobPanelProps) {
  ...
  return (
    <section className="workplace-screen">
      ...
      <section className="panel workplace-progress-panel visual-panel">
        <div className="progress-orbit" aria-hidden="true"><span>{progressPercent}%</span></div>
        <div className="section-heading">
          <div><span className="section-kicker">Профессиональный рост</span><h2>Опыт работы</h2></div>
          <strong className="progress-value">{jobExperience} / {job.promotionThreshold} XP</strong>
        </div>
        <div className="premium-progress" aria-label="Прогресс работы"><span style={{ width: `${progressPercent}%` }}><i /></span></div>
